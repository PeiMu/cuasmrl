...
def main():
    ...
    if drl_config.load is None:
        ...
        load_dir = f'{drl_config.default_out_path}/{GPU}/rmsnorm/{batch}_{heads}_{seq_len}_{dim}'
    else:
    ...
    def benchmark(NA, provider, fp8_inputs):
        ...
        if provider == 'cuasmrl':
            ms, min_ms, max_ms = triton.testing.do_bench(lambda: call(_cuasmrl, load_dir, embeddings_load, rms_weights), quantiles=quantiles, warmup=100, rep=100)
        if provider == 'triton':
