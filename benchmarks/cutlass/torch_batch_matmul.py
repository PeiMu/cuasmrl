...
def main():
    ...
    configs.append(
        triton.testing.Benchmark(
            ...
            line_vals=['cuasmrl', 'triton', 'torch'],
            line_names=['cuasmrl', 'triton', 'torch'],
    ...
    def benchmark(NA, provider, fp8_inputs):
        ...
        if provider == 'torch':
            ms, min_ms, max_ms = triton.testing.do_bench(lambda: torch.bmm(a, b), quantiles=quantiles, warmup=100, rep=100)
        if provider == 'cuasmrl':
            ms, min_ms, max_ms = triton.testing.do_bench(lambda: call(cuasmrl, load_dir, a, b), quantiles=quantiles, warmup=100, rep=100)
        if provider == 'triton':
            ms, min_ms, max_ms = triton.testing.do_bench(lambda: call_tt(tt, a, b), quantiles=quantiles, warmup=100, rep=100)
